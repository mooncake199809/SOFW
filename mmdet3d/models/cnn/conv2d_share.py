...
class Conv2d_share(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, share_type=None,
        ...
        self.score_Cout = nn.Parameter(torch.rand(self.weight.size(0), 1).cuda())
        self.score_Cin = nn.Parameter(torch.rand(1, self.weight.size(1)).cuda())

    def forward(self, input):
        score = self.score_Cout * self.score_Cin
        mean_score = score.mean().item()
        std_score = score.std().item()
        threshold = mean_score + 0.5 * std_score
        score = binarizer_fn(score, threshold)

        weight = score.unsqueeze(-1).unsqueeze(-1) * self.weight + (1 - score.unsqueeze(-1).unsqueeze(-1)) * self.specific_weight
        if self.bias is not None:
