...
class ModelNetEncoderDecoder3D(Base3DSegmentor):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(ModelNetEncoderDecoder3D, self).__init__(init_cfg=init_cfg)
    ...
    def forward_train(self, points, img_metas, pts_semantic_mask):
        ...
        if len(points[0].shape) != 3:
            ...
            points_cat = points[0]
        if len(pts_semantic_mask[0].shape) != 2:
            pts_semantic_mask_cat = torch.stack(pts_semantic_mask)
        else:
            pts_semantic_mask_cat = pts_semantic_mask[0]
    ...
    def simple_test(self, points, img_metas, rescale=True):
        ...
        x = self.extract_feat(points_cat)
        predicted_res = self.decode_head.forward_test(x, img_metas, None)   # [B 40 1]
        seg_pred = predicted_res.argmax(1)  # [B 1]

        seg_pred_res = (seg_pred == img_metas)
        seg_pred = [dict(semantic_mask = seg_pred_res)]
        return seg_pred_res
